#!/usr/bin/env pwsh
<#!
Granularity Test Script
PowerShell 7 script to benchmark RAG backend across retrieval granularities.
!>
    ...
    for ($i = 0; $i -lt $RepeatCount; $i += $Concurrency) {
        ...
        for ($j = 0; $j -lt $batch; $j++) {
            $jobs += Start-ThreadJob -ScriptBlock {
                param($u,$payload,$mode,$threads,$gran)
            ...
            } -ArgumentList $Uri, $Body, $Mode, $Threads, $Granularity
        }
        if ($jobs.Count -gt 0) {
...
$ExperimentName = "granularity"
$GranularityList = @("FINE","COARSE")
$Concurrency = 5
...
#region Execution
foreach ($gran in $GranularityList) {
    Start-RagBackend -ExecutionMode "PARALLEL" -Granularity $gran -Threads $Threads
    if (-not (Wait-ForHealth)) {
        Write-Warning "Backend failed to become healthy for granularity=$gran. Skipping."
        continue
    }
    $fileName = "granularity_${gran}.csv"
    $outputPath = Ensure-ResultPath -ExperimentName $ExperimentName -FileName $fileName
    Write-Host "Running granularity=$gran concurrency=$Concurrency threads=$Threads -> $outputPath"
    $results = Invoke-RagRequest -Uri $QueryUri -Body $RequestBody -RepeatCount $RequestsPerCondition -Concurrency $Concurrency -Mode "PARALLEL" -Threads $Threads -Granularity $gran
